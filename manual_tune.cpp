...
void manual_tune::set_inpitch(float time_begin, float time_end, const pitch_node& node)
{
    ...
    if (end + 1 > _inpitch.size())
    {
        _inpitch.resize(end + 1);
    }
    if (end + 1 > _outpitch.size())
    {
        _outpitch.resize(end + 1);
    }
...
manual_tune::pitch_node manual_tune::get_inpitch(float time)
{
    ...
    if (idx >= _max_len || idx >= _inpitch.size())
    {
        return pitch_node();
    }
...
std::list<std::pair<manual_tune::pitch_node, float> > manual_tune::get_inpitch(float time_begin, float time_end)
{
    ...
    if (end > _inpitch.size())
    {
        end = (std::uint32_t)_inpitch.size();
    }
...
manual_tune::pitch_node manual_tune::get_outpitch(float time)
{
    ...
    if (idx >= _max_len || idx >= _outpitch.size())
    {
        return pitch_node();
    }

    {
        ...
        while (idx > 0 && idx < _inpitch.size() && inpitch.is_same(_inpitch[idx - 1]))
        {
            --idx;
        }
    }
    ...
    std::shared_ptr<tune_node> tune = (idx < _tune_list.size()) ? _tune_list[idx] : nullptr;
    if (tune)
    {
        float a = 0;
        float b = 0;
...
std::list<std::pair<manual_tune::pitch_node, float> > manual_tune::get_outpitch(float time_begin, float time_end)
{
    ...
    if (end > _outpitch.size())
    {
        end = (std::uint32_t)_outpitch.size();
    }
    ...
    for (std::uint32_t i = begin; i < end; i++)
    {
        ...
        std::shared_ptr<tune_node> tune = (i < _tune_list.size()) ? _tune_list[i] : nullptr;
...
{
    std::fill(_inpitch.begin(), _inpitch.end(), pitch_node{});
}

void manual_tune::clear_outpitch()
{
    std::fill(_outpitch.begin(), _outpitch.end(), pitch_node{});
}
...
std::list<std::shared_ptr<manual_tune::tune_node> > manual_tune::get_tune(float time_begin, float time_end)
{
    ...
    if (idx_end > _tune_list.size())
    {
        idx_end = (std::uint32_t)_tune_list.size();
    }
...
{
    std::uint32_t idx = _time2idx(time);
    if (idx >= _max_len || idx >= _tune_list.size())
    {
...
{
    _save_history();
    std::fill(_tune_list.begin(), _tune_list.end(), nullptr);
}

void manual_tune::clear_auto_note()
{
    _save_history();
    for (std::uint32_t i = 0; i < _tune_list.size(); i++)
    {
...
void manual_tune::snap_key(const std::int32_t notes[12], float time_min_len, float time_max_interval, float attack, float release, float amount)
{
    ...
    for (std::uint32_t i = 0; i < _len && i < _inpitch.size(); i++)
    {
...
{
    for (std::uint32_t i = 0; i < _len && i < _inpitch.size() && i < _outpitch.size(); i++)
    {
...
void manual_tune::_write_tune(const std::shared_ptr<tune_node>& tune)
{
    ...
    if (idx_end + 1 > _tune_list.size())
    {
        _tune_list.resize(idx_end + 1, nullptr);
    }
...
void manual_tune::_remove_overlap(const std::shared_ptr<tune_node>& tune)
{
    ...
    if (idx_begin > 0 && idx_begin - 1 < _tune_list.size())
    {
...
void manual_tune::_add_auto_tune(const std::shared_ptr<tune_node>& tune)
{
    std::uint32_t idx_begin = _time2idx(tune->time_start);
    std::uint32_t idx_end = _time2idx(tune->time_end);
    std::uint32_t check_end = (idx_end < _tune_list.size()) ? idx_end : (std::uint32_t)_tune_list.size();

    for (std::uint32_t i = idx_begin; i < check_end; i++)
    {
        if (_tune_list[i] && _tune_list[i]->is_manual)
        {
            return;
...
void manual_tune::_linear_fit_from_inpitch(float time_begin, float time_end, float& a, float& b)
{
    ...
    if ((std::uint32_t)end > _inpitch.size())
    {
        end = (std::int32_t)_inpitch.size();
    }
...
{
    std::uint32_t idx_begin = 0;
    std::uint32_t idx_end = (std::uint32_t)_tune_list.size();
    std::shared_ptr<tune_node> last;
